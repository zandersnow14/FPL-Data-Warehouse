...
def get_gameweek_data() -> pd.DataFrame:
    """
    Retrieves metadata about each gameweek from the FPL API.

    :returns: Pandas DataFrame of gameweek metadata.
    """
    ...
    gw_df = gw_df[
        ['id', 'name', 'deadline_time_epoch', 'finished', 'is_current', 'is_previous', 'is_next']
        ]
...
def get_player_data() -> pd.DataFrame:
    """
    Retrieves raw player data from the FPL API.

    :returns: Pandas DataFrame containing raw player data.
    """
